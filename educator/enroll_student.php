...
// Confirm that the subject exists and is assigned to the educator
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['studentAccRefNo'])) {
    $studentAccRefNo = intval($_POST['studentAccRefNo']);

    // Double-check student is eligible
    $checkStudentSql = "SELECT accRefNo FROM student WHERE accRefNo=? AND schoolIdNo=?";
    $stmt = $conn->prepare($checkStudentSql);
    $stmt->bind_param("ii", $studentAccRefNo, $schoolIdNo);
    $stmt->execute();
    $stmt->store_result();
    if ($stmt->num_rows == 1) {
        // Not already enrolled?
        $checkEnrollSql = "SELECT enrollmentNo FROM enrolled_students WHERE subjectRefNo=? AND studentAccRefNo=? AND status='Enrolled'";
        $stmt2 = $conn->prepare($checkEnrollSql);
        $stmt2->bind_param("ii", $subjectRefNo, $studentAccRefNo);
        $stmt2->execute();
        $stmt2->store_result();
        if ($stmt2->num_rows == 0) {
            // Enroll!
            $insertSql = "INSERT INTO enrolled_students (subjectRefNo, assignedEducator, studentAccRefNo, status)
                          VALUES (?, ?, ?, 'Enrolled')";
            $stmt3 = $conn->prepare($insertSql);
            $stmt3->bind_param("iii", $subjectRefNo, $educatorRefNo, $studentAccRefNo);
            if ($stmt3->execute()) {
            ...
            $errorMsg = "Student is already enrolled in this subject.";
        }
        $stmt2->close();
    } else {
        $errorMsg = "Student not found or does not belong to your school.";
    }
    $stmt->close();
}

// Fetch eligible students (not already enrolled)
$students = [];
$sql = "SELECT s.accRefNo, s.studentNo, s.fullName, s.lrn, s.schoolIdNo, 
        ...
        LEFT JOIN (
            SELECT studentAccRefNo
            FROM enrolled_students
            WHERE subjectRefNo = ? AND status = 'Enrolled'
        ) es ON s.accRefNo = es.studentAccRefNo
        WHERE s.schoolIdNo = ? AND es.studentAccRefNo IS NULL
        ORDER BY s.fullName ASC";
$stmt = $conn->prepare($sql);
$stmt->bind_param("ii", $subjectRefNo, $schoolIdNo);
$stmt->execute();
$stmt->bind_result($accRefNo, $studentNo, $fullName, $lrn, $schoolIdNo, $parentGuardianName, $pgRStoStudent, $pgDOB, $pgMaritalStatus, $pgEmailAdd, $pgContactNo, $dob, $username);
while ($stmt->fetch()) {
    $students[] = [
        'accRefNo' => $accRefNo,
        'studentNo' => $studentNo,
...
<head>
    <meta charset="UTF-8">
    <title><?php echo htmlspecialchars($subjTitle); ?></title>   
    ...
    <style>
        ...
        .enroll-btn, .back-btn, .profile-btn {
            margin: 0 4px 3px 0;
            padding: 6px 13px;
            border-radius: 6px;
            font-size: 14px;
        ...
        .enroll-btn:hover, .back-btn:hover, .profile-btn:hover {
            background: #e6a56c;
            color: #fff;
        }
        .no-students { color: #bb741a; text-align: center; font-size: 1.1em; padding: 40px 0; }
        .msg-success { color: #228B22; font-weight:bold; text-align:center; margin:14px 0; }
        .msg-error { color: #c20800; font-weight:bold; text-align:center; margin:14px 0; }
        /* Modal Styles */
        .modal-overlay {
            display: none;
            position: fixed;
            ...
            background: rgba(10,59,82, 0.10); /* match system */
    ...
    <script>
        function confirmEnroll(studentName) {
            return confirm("Enroll " + studentName + " to this subject?");
        }
        // Modal logic
...
<div class="main-content">
    ...
    <div class="enroll-container">
        <h2 style="text-align:center; color:#c27204;">Enroll Student to "<?php echo htmlspecialchars($subjTitle); ?>"</h2>
        ...
        <a href="educator_manage_student_records.php?subjectRefNo=<?php echo urlencode($subjectRefNo); ?>" class="back-btn">Back to Students List</a>
        <?php if (count($students) > 0): ?>
        <table class="students-table">
            <thead>
                <tr>
                    <th>No</th>
                    <th>Student's Reference Number</th>
                    <th>Full Name</th>
                    <th>LRN</th>
                    <th>Actions</th>
                </tr>
            ...
            <?php $rowNo = 1; foreach ($students as $stud): ?>
                <tr>
                    ...
                    <td>
                        ...
                        <form method="post" style="display:inline;" onsubmit="return confirmEnroll('<?php echo htmlspecialchars(addslashes($stud['fullName'])); ?>');">
                            <input type="hidden" name="studentAccRefNo" value="<?php echo htmlspecialchars($stud['accRefNo']); ?>">
        ...
        <?php else: ?>
            <div class="no-students">No eligible students available for enrollment.</div>
        <?php endif; ?>
    </div>
